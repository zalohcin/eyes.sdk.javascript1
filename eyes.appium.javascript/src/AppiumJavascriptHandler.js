...
const {MouseTrigger} = require('eyes.sdk.core');
const {JavascriptHandler} = require('eyes.selenium');
...
class AppiumJavascriptHandler extends JavascriptHandler {
    ...
    handle(script, ...args) {
        ...
        if (AppiumJsCommandExtractor.isAppiumJsCommand(script)) {
            const that = this;
            return that._driver.manage().window().getSize().then(windowSize => {
                return AppiumJsCommandExtractor.extractTrigger(that._driver.getElementIds(), windowSize, that._promiseFactory, script, args);
            }).then(trigger => {
                if (trigger) {
                    // TODO - Daniel, additional type of triggers
                    if (trigger instanceof MouseTrigger) {
                        that._driver.getEyes().addMouseTrigger(trigger.getMouseAction(), trigger.getControl(), trigger.getLocation());
                    }
                }
            });
