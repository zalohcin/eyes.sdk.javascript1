...
const {ContextBasedScaleProviderFactory} = require('eyes.sdk.core');
const {EyesSeleniumUtils} = require('eyes.selenium');
...
class Eyes extends require('eyes.selenium').Eyes {
    ...
    _getScaleProviderFactory() {
        ...
        return this._positionProvider.getEntireSize().then(entireSize => {
            return EyesAppiumUtils.isMobileDevice(this._driver).then(isMobileDevice => {
                return new ContextBasedScaleProviderFactory(that._logger, entireSize, that._viewportSizeHandler.get(), that._devicePixelRatio, isMobileDevice, that._scaleProviderHandler);
            });
        });
    }
            ...
            if (!appEnv.getOs()) {
                ...
                return that._driver.getCapabilities().then(capabilities => {
                    if (EyesAppiumUtils.isMobileDeviceFromCaps(capabilities)) {
                        ...
                        if (platformName) {
                            let os = platformName;
                            let platformVersion = EyesAppiumUtils.getPlatformVersionFromCaps(capabilities);
                            if (platformVersion) {
