...
const {MouseAction, MouseTrigger} = require('eyes.sdk');
        ...
        if (script === TAP_COMMAND) {
            ...
            return promiseFactory.resolve().then(() => {
                if (elementId) {
                    ...
                    return referencedElement.getLocation().then(elementPosition => {
                        return referencedElement.getSize().then(elementSize => {
                            // If coordinates are percentage of the size of the viewport/element.
                            if (x < 1) {
                                x = elementSize.width * x;
                            }
                            if (y < 1) {
                                y = elementSize.height * y;
                            }
                ...
                const action = (tapCount === 1) ? MouseAction.Click : MouseAction.DoubleClick;
