...
class SafariScreenshotImageProvider extends ImageProvider {
  ...
  getImage() {
    ...
    return that._tsInstance
      ...
      .then(() => that._eyes.getDebugScreenshotsProvider().save(image, 'SAFARI'))
      .then(() => {
        scaleRatio = that._eyes.getDevicePixelRatio();
        return that._eyes.getViewportSize();
      })
      .then(newViewportSize => {
        viewportSize = newViewportSize.scale(scaleRatio);

        if (that._userAgent.getOS() === OSNames.IOS) {
          return image
            .crop(new Region(0, Math.ceil(64 * scaleRatio), viewportSize.getWidth(), viewportSize.getHeight()))
            .then(croppedImage => {
              image = croppedImage;
            });
        }
      ...
      .then(() => {
        if (!that._eyes.getForceFullPageScreenshot()) {
          ...
          return promise.then(loc => image.crop(new Region(loc.scale(scaleRatio), viewportSize)));
        }
