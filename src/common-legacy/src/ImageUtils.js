;(function() {
  ...
  ImageUtils.stitchImage = function stitchImage(fullSize, parts, promiseFactory) {
    return promiseFactory.makePromise(function(resolve) {
      ...
      var stitchingPromise = promiseFactory.makePromise(function(resolve) {
        resolve()
      })
      ...
      stitchingPromise
        .then(function() {
          ...
          if (actualImageWidth < stitchedImage.width || actualImageHeight < stitchedImage.height) {
            var newWidth =
              stitchedImage.width > actualImageWidth ? actualImageWidth : stitchedImage.width
            var newHeight =
              stitchedImage.height > actualImageHeight ? actualImageHeight : stitchedImage.height
            return ImageUtils.cropImage(
              stitchedImage,
              {left: 0, top: 0, width: newWidth, height: newHeight},
              promiseFactory,
            )
          }
        })
