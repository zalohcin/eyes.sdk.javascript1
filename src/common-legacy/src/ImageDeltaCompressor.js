;(function() {
   ...
   * @private
   */
  function _compareAndCopyBlocks(
    sourcePixels,
    targetPixels,
    imageSize,
    pixelLength,
   ...
   */
  ImageDeltaCompressor.compressByRawBlocks = function(
    targetData,
    targetBuffer,
    sourceData,
    blockSize,
    ...
    var blockColumnsCount = Math.floor(
      targetData.width / blockSize + (targetData.width % blockSize === 0 ? 0 : 1),
    )
    var blockRowsCount = Math.floor(
    ...
    for (var channel = 0; channel < 3; ++channel) {
      ...
      for (var blockRow = 0; blockRow < blockRowsCount; ++blockRow) {
        for (var blockColumn = 0; blockColumn < blockColumnsCount; ++blockColumn) {
          compareResult = _compareAndCopyBlocks(
            sourcePixels,
            targetPixels,
            imageSize,
            pixelLength,
    ...
    stream.write(
      zlib.deflateRawSync(blocksStream.getBuffer(), {
