    ...
    describe(`useVisualGrid: ${useVisualGrid}`, function () {
      it('TestCloseAsync', async function () {
        ...
        try {
          ...
          await eyes.open(driver, this.test.parent.title, `${this.test.title}_1`, new RectangleSize(800, 600));
          await eyes.check(null, Target.window().withName('step 1'));
          await eyes.closeAsync();

          await driver.findElement(By.css('button')).click();
          await eyes.open(driver, this.test.parent.title, `${this.test.title}_2`, new RectangleSize(800, 600));
