...
class TestDataProvider {
  ...
  static fixtureArgs() {
    const dataVariants = [];
    dataVariants.push({ options: this.getChromeOptions(), useVisualGrid: true, stitchMode: StitchMode.CSS });
    dataVariants.push({ options: this.getChromeOptions(), useVisualGrid: false, stitchMode: StitchMode.CSS });
    dataVariants.push({ options: this.getChromeOptions(), useVisualGrid: false, stitchMode: StitchMode.SCROLL });

    if (TestUtils.RUNS_ON_TRAVIS) {
      dataVariants.push({ options: this.getFirefoxOptions(), useVisualGrid: false, stitchMode: StitchMode.CSS });
      dataVariants.push({ options: this.getFirefoxOptions(), useVisualGrid: false, stitchMode: StitchMode.SCROLL });
      dataVariants.push({ options: this.getIE11Options(), useVisualGrid: false, stitchMode: StitchMode.CSS });
      dataVariants.push({ options: this.getIE11Options(), useVisualGrid: false, stitchMode: StitchMode.SCROLL });
      dataVariants.push({ options: this.getSafariOptions(), useVisualGrid: false, stitchMode: StitchMode.CSS });
      dataVariants.push({ options: this.getSafariOptions(), useVisualGrid: false, stitchMode: StitchMode.SCROLL });
    }
  ...
  static eyesBaseArgs() {
    return [
      ...
      { testedUrl: 'https://www.target.com/c/blankets-throws/-/N-d6wsb?lnk=ThrowsBlankets%E2%80%9C,tc', matchLevel: MatchLevel.Strict },
      ...
      { testedUrl: 'https://applitools.com/features/frontend-development', matchLevel: MatchLevel.Strict },
      { testedUrl: 'https://applitools.com/docs/topics/overview.html', matchLevel: MatchLevel.Strict },
