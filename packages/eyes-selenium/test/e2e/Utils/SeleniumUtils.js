  ...
  static createWebDriver(options) {
    ...
    switch (options.getBrowserName()) {
      case 'firefox':
        ...
        webDriver = new Builder().withCapabilities(Capabilities.firefox()).setFirefoxOptions(options).build();
        break;
      case 'safari':
        if (SeleniumUtils.DRIVER_PATH) {
          safari.setDefaultService(new safari.ServiceBuilder(SeleniumUtils.DRIVER_PATH).build());
        }
        webDriver = new Builder().withCapabilities(Capabilities.safari()).setSafariOptions(options).build();
        break;
      case 'internet explorer':
        if (SeleniumUtils.DRIVER_PATH) {
          ie.setDefaultService(new ie.ServiceBuilder(SeleniumUtils.DRIVER_PATH).build());
        }
        webDriver = new Builder().withCapabilities(Capabilities.ie()).setIeOptions(options).build();
        break;
      case 'edge':
        if (SeleniumUtils.DRIVER_PATH) {
          edge.setDefaultService(new edge.ServiceBuilder(SeleniumUtils.DRIVER_PATH).build());
        }
        webDriver = new Builder().withCapabilities(Capabilities.edge()).setEdgeOptions(options).build();
        break;
      default:
        if (SeleniumUtils.DRIVER_PATH) {
          chrome.setDefaultService(new chrome.ServiceBuilder(SeleniumUtils.DRIVER_PATH).build());
        }
        webDriver = new Builder().withCapabilities(Capabilities.chrome()).setChromeOptions(options).build();
        break;
    }
