  ...
  ;[{useVisualGrid: true}, {useVisualGrid: false}].forEach(({useVisualGrid}) => {
    describe(`useVisualGrid: ${useVisualGrid}`, function() {
      it('TestCheckRegionWithBadSelector', async function() {
        const {driver, runner, eyes} = SeleniumUtils.initEyes(
          useVisualGrid,
          'TestCheckRegionWithBadSelector',
        )
      ...
      it('TestCheckRegionWithBadIgnoreSelector', async function() {
        const {driver, runner, eyes} = SeleniumUtils.initEyes(
          useVisualGrid,
          'TestCheckRegionWithBadIgnoreSelector',
        )
        await driver.get('https://applitools.github.io/demo/TestPages/VisualGridTestPage/')
        ...
        await eyes.open(
          driver,
          'Applitools Eyes JS SDK',
          `TestCheckRegionWithBadIgnoreSelector${suffix}`,
          new RectangleSize(1200, 800),
        )
      ...
      it('TestCheckRegionWithBadSelectorBeforeValidCheck', async function() {
        const {driver, runner, eyes} = SeleniumUtils.initEyes(
          useVisualGrid,
          'TestCheckRegionWithBadSelectorBeforeValidCheck',
        )
        await driver.get('https://applitools.github.io/demo/TestPages/VisualGridTestPage/')
        ...
        await eyes.open(
          driver,
          'Applitools Eyes JS SDK',
          `TestCheckRegionWithBadSelectorBeforeValidCheck${suffix}`,
          new RectangleSize(1200, 800),
        )
