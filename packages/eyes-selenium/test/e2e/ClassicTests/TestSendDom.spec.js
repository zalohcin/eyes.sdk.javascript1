...
describe('TestSendDom', function() {
  ...
  it('TestSendDOM_FullWindow', async function() {
    ...
    const eyesWebDriver = await eyes.open(
      webDriver,
      'Test Send DOM',
      'Full Window',
  ...
  it('TestSendDOM_Simple_HTML', async function() {
    ...
    try {
      const eyesWebDriver = await eyes.open(
        webDriver,
        'Test Send DOM',
        'Test DomCapture method',
        new RectangleSize(1200, 1000),
      )
      const logger = new Logger()
      logger.setLogHandler(TestUtils.initLogHandler())
      const actualDomJson = await DomCapture.getFullWindowDom(logger, eyesWebDriver)
      const expectedDomJson = await getExpectedDomFromUrl(
        'https://applitools-dom-capture-origin-1.surge.sh/test.dom.json',
      )
  ...
  it('TestSendDOM_2', async function() {
    await _captureDom(
      'https://applitools.github.io/demo/TestPages/DomTest/dom_capture_2.html',
      this.test.title,
    )
  })
})
