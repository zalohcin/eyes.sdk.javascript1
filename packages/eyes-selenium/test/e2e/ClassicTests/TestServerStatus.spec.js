...
describe('TestServerStatus', function () {
  ...
  it('TestSessionSummary_Status_Failed', async function () {
    ...
    await eyes.open(webDriver, `TestServerStatus${guid}`, `TestServerStatus${guid}`, new RectangleSize(800, 600));
    ...
    await eyes.open(webDriver, `TestServerStatus${guid}`, `TestServerStatus${guid}`, new RectangleSize(800, 600));
    await webDriver.get('https://applitools.github.io/demo/TestPages/DynamicResolution/desktop.html');
    try {
      await eyes.check(`TestSessionSummary_Status_Failed${guid}`, Target.window().fully(false));
  ...
  it('TestSessionSummary_Status_New', async function () {
    ...
    const guid = `_${GeneralUtils.guid()}`;
    const driver = await eyes.open(webDriver, `TestServerStatus${guid}`, `TestServerStatus${guid}`, new RectangleSize(800, 600));
