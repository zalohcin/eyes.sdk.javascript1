...
describe('TestScrollRootElement', function () {
  ...
  dataProvider.forEach((row) => {
    testSetup = new TestSetup('Eyes Selenium SDK - Scroll Root Element', row.options, row.useVisualGrid, row.stitchMode);
    testSetup.setTestedPageUrl('https://applitools.github.io/demo/TestPages/SimpleTestPage/scrollablebody.html');

    describe(testSetup.toString(), function () {
      it('TestCheckWindow_Body', async function () {
        await testSetup.getEyes().check(`Body (${testSetup.getEyes().getStitchMode()} stitching)`, Target.window()
