  ...
  [
    ...
    describe(`stitchMode: ${stitchMode}`, function () {
      it('TestCloseAsync', async function () {
        ...
        let driver;
        try {
          driver = await new Builder().usingServer(TestDataProvider.BROWSERSTACK_SELENIUM_URL).withCapabilities(options).build();
          await driver.get('https://applitools.github.io/demo/TestPages/FramesTestPage/');
          eyes.setStitchMode(stitchMode);
          eyes.setBatch(TestDataProvider.BatchInfo);
          await eyes.open(driver, 'TestBrowserStack', `TesIE11_${stitchMode}`, new RectangleSize(800, 600));
          await eyes.check('viewport', Target.window().fully(false).sendDom(false));
          await eyes.check('full page', Target.window().fully(true).sendDom(false));
          await eyes.close();
        } finally {
          await driver.quit();
          await eyes.abort();
