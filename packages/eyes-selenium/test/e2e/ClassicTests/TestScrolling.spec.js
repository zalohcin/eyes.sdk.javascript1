    ...
    describe(`runMethod ${runMethod}`, async function () {
      it('TestWebAppScrolling', async function () {
        ...
        switch (runMethod) {
          case RunMethod.SauceLabs: {
            ...
            driver = await new Builder().usingServer(TestDataProvider.SAUCE_SELENIUM_URL).withCapabilities(options2).build();
            break;
          }
          default: {
        ...
        switch (runMethod) {
          ...
          default: {
            ...
            options.setMobileEmulation({
              ...
              height: 512,
              pixelRatio: 4.0,
              userAgent: 'Mozilla/5.0 (Linux; Android 7.1.1; Nexus 9) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/70.0.3538.110 Safari/537.36',
            });
            driver = SeleniumUtils.createChromeDriver(options);
            break;
          }
      ...
      it('TestWebAppScrolling3', async function () {
        ...
        try {
          ...
          await eyes.check('long page on mobile', Target.region(By.css('div.page')).fully(false).sendDom(false));
          await eyes.close();
        } finally {
          await eyes.abort();
          await driver.quit();
