...
const { Eyes, Target, RectangleSize, Region, VisualGridRunner, ClassicRunner, BrowserType } = require('../../../index');
  ...
  [
    ...
    it(`HelloWorldTest {useVisualGrid: ${useVisualGrid}}`, async function () {
      ...
      try {
        ...
        // Add 2 checks
        await eyes.check(null, Target.window().withName('Step 1 - Viewport').ignoreRegions(By.css('.primary')));
        await eyes.check(null, Target.window().fully().withName('Step 1 - Full Page')
        ...
        await eyes.check(null, Target.window().fully().withName('Step 2 - Full Page'));
