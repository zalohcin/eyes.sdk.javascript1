  ...
  dataProvider.forEach(row => {
    ...
    describe(testSetup.toString(), function() {
      ...
      it('TestCheckRegion2', async function() {
        await testSetup
          .getEyes()
          .checkRegionBy(By.id('overflowing-div-image'), 'minions', undefined, true)
      })
      ...
      it('TestCheckInnerFrame', async function() {
        ...
        await testSetup
          .getDriver()
          .switchTo()
          .frame(await testSetup.getDriver().findElement(By.name('frame1')))
        ...
        await testSetup
          .getDriver()
          .executeScript("arguments[0].style.background='red';", innerFrameBody)
