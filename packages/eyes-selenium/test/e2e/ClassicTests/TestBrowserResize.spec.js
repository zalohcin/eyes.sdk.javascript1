...
describe('TestBrowserResize', function() {
  ...
  it('BrowserSizeTest', async function() {
    ...
    try {
      ...
      assert.strictEqual(
        new RectangleSize(640, 480),
        sessionResults1.getActualAppOutput()[0].getImage().length,
      )

      assert.strictEqual(1, sessionResults2.getActualAppOutput().length)
      assert.strictEqual(
        new RectangleSize(800, 600),
        sessionResults2.getActualAppOutput()[0].getImage().length,
      )

      assert.strictEqual(1, sessionResults3.getActualAppOutput().length)
      assert.strictEqual(
        new RectangleSize(1024, 768),
        sessionResults3.getActualAppOutput()[0].getImage().length,
      )
    } finally {
