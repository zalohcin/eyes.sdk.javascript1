...
describe('TestAcme', function () {
  ...
  dataProvider.forEach((row) => {
    testSetup = new TestSetup('Eyes Selenium SDK - ACME', row.options, row.useVisualGrid, row.stitchMode);
    ...
    describe(testSetup.toString(), function () {
      it('TestAcmeTable', async function () {
        ...
        await testSetup.getEyes().check('main window with table',
          Target.window()
            .fully()
            .ignoreRegions(By.className('toolbar'))
            .layoutRegions(By.id('orders-list-desktop'), By.className('snapshot-topic'), By.id('results-count'))
            .strict());
      });
