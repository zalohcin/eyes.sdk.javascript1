  ...
  dataProvider.forEach(row => {
    ...
    describe(testSetup.toString(), function() {
      ...
      it('TestCheckRegionByCoordinates_Fluent', async function() {
        await testSetup
          .getEyes()
          .check('Fluent - Region by coordinates', Target.region(new Region(50, 70, 90, 110)))
      })
      ...
      it('TestCheckElementWithIgnoreRegionByElementOutsideTheViewport_Fluent', async function() {
        ...
        await testSetup
          .getEyes()
          .check('Fluent - Region by element', Target.region(element).ignoreRegions(ignoreElement))
      ...
      it('TestCheckElementWithIgnoreRegionBySameElement_Fluent', async function() {
        ...
        await testSetup
          .getEyes()
          .check('Fluent - Region by element', Target.region(element).ignoreRegions(element))
      ...
      it('TestCheckScrollableModal', async function() {
        testSetup
          .getDriver()
          .findElement(By.id('centered'))
          .Click()
        const scrollRootSelector =
          row.stitchMode === StitchMode.CSS ? By.id('modal-content') : By.id('modal1')
        await testSetup.getEyes().check(
          'Scrollable Modal',
      ...
      it('TestCheckWindowWithFloatingByRegion_Fluent', async function() {
        await testSetup
          ...
          .check(
            ...
            Target.window().floatingRegion(new Region(10, 10, 20, 20), 3, 3, 20, 30),
          )
      ...
      it('TestCheckElementFully_Fluent', async function() {
        ...
        await testSetup
          .getEyes()
          .check('Fluent - Region by element - fully', Target.region(element).fully())
      })

      it('TestSimpleRegion', async function() {
        await testSetup
          .getEyes()
          .check(undefined, Target.window().region(new Region(50, 50, 100, 100)))
      })
      ...
      it('TestAccessibilityRegions', async function() {
        ...
        testSetup.setExpectedAccessibilityRegions(
          new AccessibilityMatchSettings({
            ...
            width: 456,
            height: 306,
            type: AccessibilityRegionType.LargeText,
          }),
          new AccessibilityMatchSettings({
            left: 8,
            top: 1270,
