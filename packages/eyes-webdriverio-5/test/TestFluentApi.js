...
shared.examplesFor('TestFluentApi', function(test) {
  ...
  it('TestCheckRegionWithIgnoreRegion_Fluent', async () => {
    await test.eyes.check(
      ...
      Target.region(By.id('overflowing-div')).ignore(new Region(50, 50, 100, 100)),
    )
  })
  ...
  it.skip('TestCheckRegionInFrame2_Fluent', async () => {
    ...
    const result2 = await test.eyes.check(
    ...
    const result3 = await test.eyes.check(
    ...
    const result4 = await test.eyes.check(
    ...
    const result5 = await test.eyes.check(
    ...
    strictEqual(
      result1.getAsExpected() &&
        result2.getAsExpected() &&
        result3.getAsExpected() &&
        result4.getAsExpected() &&
        result5.getAsExpected(),
      true,
    )
  })
  ...
  it.skip('TestCheckFrameInFrame_Fully_Fluent2', async () => {
    const result1 = await test.eyes.check(
      'Fluent - Window with Ignore region 2',
      Target.window().fully(),
    )
    const result2 = await test.eyes.check(
    ...
    strictEqual(result1.getAsExpected() && result2.getAsExpected(), true)
  })
  ...
  it.skip('TestCheckWindowWithFloatingByRegion_Fluent', async () => {
    ...
    strictEqual(result.getAsExpected(), true)
    test.setExpectedFloatingsRegions(
  ...
  it.skip('TestCheckElementFully_Fluent', async () => {
    const webElement = await test.eyes
      .getDriver()
      .webDriver.findElement(By.id('overflowing-div-image'))
    const result = await test.eyes.check(
      'Fluent - Region by element - fully',
      Target.region(webElement).fully(),
    )
    strictEqual(result.getAsExpected(), true)
  })
  ...
  it.skip('TestCheckElementWithIgnoreRegionByElementOutsideTheViewport_Fluent', async () => {
    const element = await test.eyes
      .getDriver()
      .webDriver.findElement(By.id('overflowing-div-image'))
    const ignoreElement = await test.eyes
      .getDriver()
      .webDriver.findElement(By.id('overflowing-div'))
    const result = await test.eyes.check(
      'Fluent - Region by element - fully',
      Target.region(element).ignore(ignoreElement),
    )
    strictEqual(result.getAsExpected(), true)
  })
  ...
  it.skip('TestCheckElementWithIgnoreRegionBySameElement_Fluent', async () => {
    const webElement = await test.eyes
      ...
      .webDriver.findElement(By.id('overflowing-div-image'))
    const result = await test.eyes.check(
      'Fluent - Region by element',
      Target.region(webElement).ignore(webElement),
    )
    strictEqual(result.getAsExpected(), true)
