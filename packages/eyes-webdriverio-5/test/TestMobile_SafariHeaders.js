...
describe.skip('IOSTest', function() {
  ...
  const dataProvider = []
  dataProvider.push(
    ...TestUtils.cartesianProduct('iPhone X Simulator', ['portrait', 'landscape'], '11.0', [
      false,
      true,
    ]),
  )
  ...
  dataProvider.forEach(row => {
    ...
    it(testName, async function() {
      ...
      const result = await eyes.check(
        'Initial view',
        Target.region(By.cssSelector('body')).fully(fully),
      )
      return strictEqual(result.getAsExpected(), true)
    })
  })
