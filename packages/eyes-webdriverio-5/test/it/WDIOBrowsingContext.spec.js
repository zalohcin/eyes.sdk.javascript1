...
describe('WDIOBrowsingContext', function() {
  ...
  async function getDocument(browser) {
    const element = await browser.execute('return window.document')
    return element
  }

  async function getFrameElement(browser) {
    const element = await browser.execute('return window.frameElement')
    return element
  }
  ...
  it('frame(frameIndex)', async () => {
    const frameIndex = 1
    const frameElements = await browser.$$('frame, iframe')
  ...
  it('frame(frameId)', async () => {
    ...
    const frameElement = await browser.$(`iframe#${frameId}`)
  ...
  it('frame(element)', async () => {
    const frameElement = await browser.$('iframe#frame_main')
