;(function() {
  ...
  GeometryUtils.isRegionsIntersected = function(region1, region2) {
    ...
    return (
      ((region1.left <= region2.left && region2.left <= aRight) ||
        (region2.left <= region1.left && region1.left <= bRight)) &&
      ((region1.top <= region2.top && region2.top <= aBottom) ||
        (region2.top <= region1.top && region1.top <= bBottom))
    )
  }
  ...
  GeometryUtils.isRegionContainsRegion = function(region1, region2) {
    ...
    return (
      this.top <= region2.top &&
      this.left <= region2.left &&
      bottom >= otherBottom &&
      right >= otherRight
    )
  }
