...
const {makeGetConfig, toEnvVarName} = require('../lib/config');
...
describe('config', () => {
  ...
  function makeGetConfigAtConfigPath(args) {
    const cwd = process.cwd();
    process.chdir(configPath);
    const getConfig = makeGetConfig(args);
    process.chdir(cwd);
    return getConfig;
  }
  ...
  it('loads config with env variables', () => {
    ...
    const getConfig = makeGetConfigAtConfigPath({configParams: ['bla']});
    const config = getConfig();
