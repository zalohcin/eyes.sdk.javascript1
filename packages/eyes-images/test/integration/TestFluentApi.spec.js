...
describe('EyesImages.TestFluentApi', function () {
  ...
  it('TestImage', async function () {
    await eyes.check(this.test.title, Target.image(`${__dirname}/../resources/minions-800x500.png`));
    await eyes.close();
  });
  ...
  it('TestRegion', async function () {
    await eyes.check(this.test.title, Target.region(`${__dirname}/../resources/minions-800x500.png`, new Region({ left: 315, top: 290, width: 190, height: 135 })));
    await eyes.close();
  });
