...
Cypress.Commands.add('eyesCheckWindow', args => {
  ...
  const layoutBreakpoints = args && args.layoutBreakpoints || eyesOpenArgs && eyesOpenArgs.layoutBreakpoints || globalBreakpoints;
  const checkArgs = {};
  ...
  return cy.document({ log: false }).then({ timeout: 60000 }, async doc => await eyesCheckWindow(doc, checkArgs));
});
