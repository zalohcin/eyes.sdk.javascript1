...
class TestSetup {
  constructor(
    testClassName,
    testSuitName,
    testedPage = 'http://applitools.github.io/demo/TestPages/FramesTestPage/',
  ) {
  ...
  async beforeMethod(methodName, beforeOpen) {
    ...
    const extendedTestName = `${testName}_${this._caps.getBrowserName()}_${
      this._platform
    }_${DateTimeUtils.toLogFileDateTime()}`

    // noinspection EmptyCatchBlockJS
    this._webDriver = await new Builder()
      .withCapabilities(this._caps)
    ...
    if (process.env.CI == null && this._logsPath != null) {
      ...
      logHandler = new FileLogHandler(
        true,
        `${logPath}${path.sep}${testName}_${this._platform}.log`,
        true,
      )
    ...
    this._driver = await this._eyes.open(
      this._webDriver,
      this._testSuitName,
      testName,
      this._testedPageSize,
    )
  ...
  toString() {
    return `${this._testClassName} (${this._caps.getBrowserName()}, ${this._platform}, force FPS: ${
      this._forceFPS
    })`
  }
}
