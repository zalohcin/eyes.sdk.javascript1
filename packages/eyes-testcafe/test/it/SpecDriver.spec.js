...
test('mainContext()', driver => {
  return mainContext({driver})
})
test.skip('parentContext()', driver => {
  return parentContext({driver})
})
test.skip('childContext(element)', async driver => {
  childContext({driver})
})
test('getTitle()', driver => {
  return getTitle({driver})
})
test('getUrl()', driver => {
  return getUrl({driver, expected: 'https://applitools.github.io/demo/TestPages/FramesTestPage/'})
})
...
async function parentContext({_driver}) {
  ...
  //  await driver.expect(parentDocument.visible).ok()
  //  await driver.switchToIframe('[name="frame1-1"]')
  //  const frameDocument = await Selector('html')()
  //  assert.ok(!spec.isEqualElements(driver, parentDocument, frameDocument))
  //  //await spec.parentContext(driver)
  //  const elSnapshot = await frameDocument.selector.parent()()
  //  const r = spec.helpers.extractSelectorString(elSnapshot)
  //  debugger
  //  await driver.switchToIframe(elSnapshot)
  //  const resultDocument = await Selector('html')()
...
async function getUrl({driver, expected}) {
  const result = await spec.getUrl(driver)
  assert.deepStrictEqual(result, expected)
}
async function visit({driver}) {
  try {
