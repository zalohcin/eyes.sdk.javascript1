    ...
    for (const [requiredWidth, browsersInfo] of requiredWidths.entries()) {
      ...
      try {
        ...
        const message = isStrictBreakpoints
          ...
          : chalk.yellow(
              `Following configurations [${failedBrowsers}] have a viewport-width which is not able to be set on your local browser, because it has a limit of ${actualViewportSize.getWidth()} pixels. As a fallback, the resources that will be used for this checkpoint have been captured on the browser's limit (${actualViewportSize.getWidth()} pixels). To resolve this, you may use a headless browser as it can be resized to any size.`,
            )
        console.log(message)
      }
      const snapshot = await takeDomSnapshot({
