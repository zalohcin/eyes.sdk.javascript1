...
class EyesRemoteWebElement {
    ...
    static registerSendKeys(element, eyesDriver, logger, args) {
        ...
        return _getBounds(element).then(re => {
            eyesDriver.getEyes().addTextTrigger(re, text);
        });
    }
    ...
    static registerClick(element, eyesDriver, logger) {
        ...
        return _getBounds(element).then(rect => {
    ...
    getComputedStyleInteger(propStyle) {
        return this.getComputedStyle(propStyle).then(value => Math.round(parseFloat(value.trim().replace("px", ""))));
    }
    ...
    /**
     * @return {!promise.<{width: number, height: number}>} element's size
     */
    getSize() {
        return this._element.getSize().then(value => GeometryUtils.createSize(value.width, value.height));
    }

    /**
     * @return {!promise.<{x: number, y: number}>} element's location
     */
    getLocation() {
...
function _getRectangle(location = {x: 0, y: 0}, size = {height: 0, width: 0}) {
    let left = location.x, top = location.y, width = size.width, height = size.height;
